...
const reactive (target = {}) {
  ...
  const proxyConf = {
    get(target, key, receiver) { // receiver实际是proxyData，继承Prxoy的对象
      // 非原型的属性才进行处理
      const ownKeys = Reflect.ownKeys(target)
      if (ownKeys.includes(key)) {
        // 在这里加自己的处理
      }
      const result = Reflect.get(target, key, receriver)
return reactive(result)
    },
    set(target, key, val, receiver) {
      // 重复数据不处理
      const oldVal = taget[key]
      ...
      const result = Reflect.set(target, key, val, receriver)
      return result
    },
